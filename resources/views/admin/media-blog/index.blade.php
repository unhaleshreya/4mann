                ...
                <div class="card-body">
                    <form action="{{ route('media-blog.store') }}" id="blogForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="html_content" id="summernote" class="form-control" rows="5"></textarea>
                        </div>
...
$(document).on("click", ".editBlogBtn", function () {
   ...
   success: function (response) {
    ...
    if (response.image) {
        let imageUrl = "{{ asset('storage/') }}/" + response.image;
        $("#currentImagePreview").html(`
            <p>Current Image:</p>
            <img src="${imageUrl}" alt="Current Image" class="img-fluid" style="max-width: 200px; border: 1px solid #ccc; padding: 5px;">
        `);
    } else {
        $("#currentImagePreview").html('');
    }
    ...
    // Scroll to form
    $('html, body').animate({
        scrollTop: $("#blogForm").offset().top - 20
    }, 600);
}
    });
});
